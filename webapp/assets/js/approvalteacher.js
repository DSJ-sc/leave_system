	...
	function computerShow(){
		$.ajax({
		     async : true ,
		     type : "post",//传值方式
		     url : 'getApprovalTeacherLeave',//请求的地址
		     data : { 	
		     },
		     //回调函数 返回的值
	...
	function Approval(sign,leaveNum){
		 $.ajax({
		     async : true ,
		     type : "post",//传值方式
		     url : 'setApprovalTeacherLeave',//请求的地址
		     data : { 
		      //传送的数据 名:值用逗号隔开
		     "leaveId" : leaveNum,
		     "approval":sign
 ...
 function showInfo(){
		...
		teacherName.val(teacherLeave[0].teacherName);
		...
		replaceTeacher.val(teacherLeave[0].replaceTeacher);
		console.log(teacherLeave[0]);
